...
extension DictionaryEncoder {
  public func encodeWWWFormUrlEncodedBody<T: Encodable>(_ v: T) throws(NetworkingError) -> String {
    do {
...
extension Networking {
  public func wwwFormUrlEncodedBody<T: Encodable>(for body: T) throws(NetworkingError) -> String {
    do {
      #if canImport(DictionaryCoding)
      return try dictionaryEncoder.encodeWWWFormUrlEncodedBody(body)
      #else
      fatalError()
      #endif
    } catch {
...
extension Networking where Request == URLRequest {
  ...
  func baseRequest<E>(_ endpoint: E) throws(NetworkingError) -> Request where E: Endpoint {
    endpoint.check()
    #if NETWORKING_LOGGING
    if #available(macOS 11.0, iOS 14.0, watchOS 7.0, tvOS 14.0, *) {
